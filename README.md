...
Test OpenXTalk Stack showing a few examples of usage of FontTools.lcb. There's also some examples of using the built-in Font functions. This can be useful for doing things like copying invisible Font-Sync-cache files from hidden directories that certain graphics company software uses to store on the fly online font syncing caches.
